...
flags.DEFINE_string('video', './data/video/test3.mp4', 'path to input video or set to 0 for webcam')
flags.DEFINE_string('output', './outputs/tracker3.avi', 'path to output video')
...
def iterate(lines, model, vid, frame_num):
    ...
    for track in tracker.tracks:
        ...
        tracks.append(Rect(track.track_id, (int(bbox[0]), int(bbox[1])), (int(bbox[2]) - int(bbox[0]), int(bbox[3]) - int(bbox[1]))))
    ...
    for line in lines:
        ...
        cv2.putText(frame, str(line.count), line.center, cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2);

    cv2.putText(frame, str(frame_num), (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 0), 2);
